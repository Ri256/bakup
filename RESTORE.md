...
⚠️ Репозиторий **приватный**, поэтому скачать скрипт одной командой нельзя —
`raw.githubusercontent.com` требует авторизации (вернёт 404).

Вариант А: сделать репозиторий публичным → тогда работает:
```bash
curl -L https://raw.githubusercontent.com/Ri256/bakup/main/setup.sh | bash
```

Вариант Б (приватный репо): в браузере откройте
`https://github.com/Ri256/bakup/blob/main/setup.sh` → **Raw** →
скопируйте текст в `~/setup.sh`, затем:
```bash
